package com.sedatbsp.asu.infrastructure.configuration.localization;
...
public class LocalizationConfiguration extends AcceptHeaderLocaleContextResolver implements WebMvcConfigurer {
    ...
    private static final List<String> SUPPORTED_LANGUAGE = List.of("en_US");

    @Bean
    public ResourceBundleMessageSource messageSource() {
        ResourceBundleMessageSource source = new ResourceBundleMessageSource();
        source.setBasenames("i18n/BusinessMessages");
    ...
    public LocaleContext resolveLocaleContext(ServerWebExchange exchange) {
        List<String> languages = Optional
                .ofNullable(exchange.getRequest().getHeaders().get(LANGUAGE_HEADER_NAME))
